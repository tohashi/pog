...
(function() {
  ...
  $(function() {
    return React.renderComponent(POG.Home(null), $('.js-main-container').get(0));
  });
  ...
  POG.Home = React.createClass({
    ...
    render: function() {
      ...
      React.DOM.div({className: "text-center"}, 
        ...
        React.DOM.p(null, 
          React.DOM.a({href: "#", className: "btn btn-primary btn-lg", onClick: this.onClick}, "Add New")
        )
      ), 

      POG.Piles({url: "/api/pile"}), 

      React.DOM.h5(null, "Ranking(24h)"), 
      POG.Ranking({url: "/api/ranking/day"}), 

      React.DOM.h5(null, "Ranking(total)"), 
...
(function() {
  POG.Ranking = React.createClass({
    ...
    render: function() {
      var rankingNodes;
      rankingNodes = this.state.data.map(function(ranking) {
        var content, url;
        content = ranking.content;
        url = "/contents/" + content.id;
        return React.DOM.a({href: url}, 
        React.DOM.li({className: "list-group-item"}, 
          React.DOM.span({className: "badge"}, ranking.count), 
          content.name
        )
      );
      });
...
(function() {
  POG.Piles = React.createClass({
    ...
    onClick: function(e) {
      e.preventDefault();
      return $('.js-edit-modal').modal();
    },
    render: function() {
      var pileNodes;
      pileNodes = this.state.data.map((function(pile) {
        var listClassName, platformNodes;
        platformNodes = pile.platforms.map(function(platform) {
          return React.DOM.div({className: "badge platform-badge"}, platform.name);
        });
        listClassName = (function() {
          return 'pile-list list-group-item clearfix ' + (function() {
            switch (pile.status) {
              case 0:
                return 'bg-piling';
              case 1:
        ...
        return React.DOM.li({className: listClassName, onClick: this.onClick}, 
        React.DOM.div({className: "pull-left"}, 
          React.DOM.div({className: "list-group-item-heading"}, 
            platformNodes, 
            React.DOM.span({className: "pile-title"}, pile.content.name)
          ), 
          React.DOM.p({className: "pile-memo list-group-item-text"}, pile.memo)
        ), 

        React.DOM.div({className: "pull-right"}, 
          React.DOM.p({className: "list-group-item-text"}, pile.last_updated, " ago")
        )
      );
      }).bind(this));
      return React.DOM.div(null, 
